...
- DONE (Live integrations & overhauls):
  ...
  9. Scheduled Automation: Nightly cron at 02:00 AM IST (20:30 UTC) in vercel.json during peak model succe  ss and low traffic.
  ...
  21. Daily Digest & Scout Automation Resilience: Fixed daily email outage by applying migration 007 (`digest_sent_at`) directly to `data/seed_data.db` and guaranteeing `db.migrate()` runs on serverless boot in `api/index.py`. Restructured `/api/cron/sync-and-match` with email-first guaranteed dispatch before matcher scoring, preventing Vercel HTTP 504 timeouts. Added `/a/scout/run` on-demand route, UI control buttons in `/jobs` and `/system`, and wired digest dispatch + live Vercel webhook triggering into `.github/workflows/daily.yml`.
- NOT DONE — next steps:
